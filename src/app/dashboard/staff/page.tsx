...
export default function StaffPage() {
  ...
  const handleSaveStaff = async () => {
    ...
    try {
      if (currentStaff.id) {
        ...
        // This is for adding a NEW staff member from the dashboard
        const staffData = {
          name: currentStaff.name,
          title: currentStaff.title,
          avatarUrl: currentStaff.avatarUrl || `https://placehold.co/40x40.png`,
          userId: user.uid,
          availability: defaultAvailability, // Always set default availability for new staff
        };
  ...
  const openAvailabilityDialog = (staffMember: StaffMember) => {
      const initialAvailability = daysOfWeek.map(day => {
          ...
          // If availability is missing for any reason, fall back to a safe default
                      ...
                      <div key={avail.day} className="grid grid-cols-4 items-center gap-4">
                          <div className="flex items-center space-x-2">
                              <Checkbox 
                                  ...
                                  onCheckedChange={(checked) => updateAvailability(avail.day, 'isWorking', Boolean(checked))}
                              />
