...
export default function BarberBookingPage() {
    ...
    useEffect(() => {
        ...
        for (const dayKey in dailyBookings) {
            ...
            staff.forEach(staffMember => {
                // Defensive check for availability
  ...
  useEffect(() => {
    ...
    // **Robustness Fix**: Ensure staff member has availability defined before proceeding
    if (!booking.staff.availability || booking.staff.availability.length === 0) {
        console.warn(`Staff member ${booking.staff.id} has no availability defined.`);
        setAvailableTimes([]);
        return;
    }
    ...
    // Filter out already booked slots for this staff member on this day
    const bookedTimes = bookings
        .filter(b => 
            b.staffId === booking.staff!.id &&
            isSameDay(b.bookingTime.toDate(), booking.date)
        )
        .map(b => format(b.bookingTime.toDate(), 'HH:mm'));
  ...
  }, [booking.staff, booking.date, booking.service, timeOff, bookings]);
                 ...
                 {isLoading ? (
                    ...
                    <fieldset className="space-y-4" disabled={!booking.service || !booking.staff}>
                        ...
                        <Calendar
                            ...
                            onSelect={(date) => date && setBooking({ ...booking, date: date })}
